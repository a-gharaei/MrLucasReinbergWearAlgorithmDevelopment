...
from PhysicalModels.controller import PidControllerResultList
from PhysicalModels.drive_system import (
    DriveSystemConfiguration,
    calculate_spindle_signals,
)
from PhysicalModels.force_controlled_kinematics import ProportionalControllerResultList
from PhysicalModels.kienzle_force_model import MaterialRemovalResultsList
from PhysicalModels.takazawa_temperature_model import TemperatureResultList
from PhysicalModels.Usui_wear_model import SimulationWearModelResults
from SimulationToolbox.Simulation.physical_models import ToolForceModelResultListFloat

from AnalyzeSimulationResults.visualizing_results import *
...
def result_extraction(
    ...
    number_of_available_experiment_folders = 2
    for exp in range(number_of_available_experiment_folders):
        ...
        if os.path.isfile(os.path.join(PKLs_path, "wheel_properties.csv")):
            wheel_properties_df = pd.read_csv(
                os.path.join(PKLs_path, "wheel_properties.csv")
            )
        ...
        if os.path.isfile(os.path.join(PKLs_path, "simulation_controller_results.pkl")):
            controller_df = PidControllerResultList.load_from_disk(
                os.path.join(PKLs_path, "simulation_controller_results.pkl")
            ).get_in_dataframe()
        ...
        if os.path.isfile(
            os.path.join(PKLs_path, "simulation_force_controlled_results.pkl")
        ):
        ...
        # -------------------------------forces----------------------------
        if os.path.isfile(
            os.path.join(PKLs_path, "simulation_tool_forces_global_frame.pkl")
        ):
        ...
        if os.path.isfile(
            ...
            for (
            ...
            ) in simulation_temperature_results.temperature_results_list:
                ...
                melting_point_exceeding_threshold = float(
                    input_parameter_df["workpiece_melting_point[K]"][exp]
                    - input_parameter_df["ambient_temperature[K]"][exp]
                )
                grains_with_temperature_above_melting_point = list(
                    filter(
                        lambda x: x > melting_point_exceeding_threshold,
                        temperature_model_result.grains_temperature_rise,
                    )
                )

                number_grains_with_temp_above_ambient.append(
                    len(grains_with_temperature_above_ambient)
                )
                number_grains_with_temp_above_melting_point.append(
                    len(grains_with_temperature_above_melting_point)
                )
                ...
                if len(grains_with_temperature_above_ambient) > 0:
                    average_grains_temperature_rise.append(
                        sum(grains_with_temperature_above_ambient)
                        / len(grains_with_temperature_above_ambient)
                    )
                else:
                    average_grains_temperature_rise.append(int(0))
                first_grain_temperature_rise.append(
                    temperature_model_result.grains_temperature_rise[0]
                )
            ...
            temperature_df = pd.DataFrame.from_dict(
                {
                    ...
                    "Sum of all material-removing grains' temperature rise - K": sum_of_temperature_rise_of_grains_with_temp_above_ambient,
        ...
        if os.path.isfile(
            ...
            for (
                mat_removal_result
            ) in material_removal_results.material_removal_result_list:
                removed_volumes_sum.append(sum(mat_removal_result.removed_volumes))

                grains_projected_area_filtered = list(
                    filter(lambda x: x > 0, mat_removal_result.projected_areas)
                )
                projected_area_sum.append(sum(grains_projected_area_filtered))
                number_of_grains_removing_material.append(
                    len(grains_projected_area_filtered)
                )
                first_grain_penetration_depth.append(
                    mat_removal_result.penetration_depths[0]
                )
            material_removal_df = pd.DataFrame.from_dict(
                {
                    "Sum of removed volumes - mm3": removed_volumes_sum,
                    "Sum of projected cutting areas - mm2": projected_area_sum,
                    "No. of material-removing grains": number_of_grains_removing_material,
            ...
            material_removal_df["Sum of projected cutting areas - micrometer^2"] = (
                1000
                * 1000
                * material_removal_df["Sum of projected cutting areas - mm2"]
            )
            material_removal_df[
                "Cumulative removed volume - mm3"
            ] = material_removal_df["Sum of removed volumes - mm3"].cumsum()
        ...
        if os.path.isfile(
            ...
            drive_sys_config = DriveSystemConfiguration.from_config_file(
                path=r"PhysicalModels/configs.yaml"
            )
            ...
            for tangential_force in tool_forces_y:
                spindle_signal_results = calculate_spindle_signals(
                    tangential_force,
                    input_parameter_df["rotational_speed[Hz]"][exp].item() * 2 * np.pi,
                    wheel_properties_df["average_radius"].item(),  # [m]
                    drive_sys_config,
                )

                spindle_current_results.append(spindle_signal_results.spindle_current)
                spindle_mechanical_power_results.append(
                    spindle_signal_results.mechanical_power_of_the_spindle
                )
                spindle_electrical_power_results.append(
                    spindle_signal_results.electrical_power_of_the_spindle
                )

            spindle_df = pd.DataFrame.from_dict(
                {
                    "Spindle electrical current - A": spindle_current_results,
            ...
            if os.path.isfile(os.path.join(PKLs_path, "simulation_wear_results.pkl")):
                ...
                for (
                    wear_model_result
                ) in simulation_wear_results.simulation_wear_model_results:
                    filtered_wear_magnitude = list(
                        filter(lambda x: x > 0, wear_model_result.wears_magnitude)
                    )
                    number_grains_subject_to_attritious_wear.append(
                        len(filtered_wear_magnitude)
                    )
                    ...
                    first_grain_attritious_wear.append(
                        wear_model_result.wears_magnitude[0]
                    )
                ...
                attritious_wear_df[
                    "Average of magnitude of grains' attritious 3D wear vectors - micrometer"
                ] = (
                    1000
                    * attritious_wear_df[
                        "Average of magnitude of grains' attritious 3D wear vectors - mm"
                    ]
                )
                attritious_wear_df[
                    "Sum of magnitude of grains' attritious 3D wear vectors - micrometer"
                ] = (
                    1000
                    * attritious_wear_df[
                        "Sum of magnitude of grains' attritious 3D wear vectors - mm"
                    ]
                )
                attritious_wear_df[
        ...
        time_step_size = float(
            1
            / (
                input_parameter_df["simulation_step_size[steps/rev]"][exp]
        ...
        report_df["Time - mSec"] = (
            1000 * time_step_size * report_df["Simulation steps"]
        )  # mSec
        ...
        if extract_figures == True:
            ...
            save_all_labels_in_TIFF_format(
                report_df,
...
if __name__ == "__main__":
    input_dir = r"MultipleRunSimulations/Results"
    number_of_available_experiment_folders = 2
    input_parameter_df = pd.read_csv(r"MultipleRunSimulations/input_parameters.csv")

    visualization_config = {
        "visualize_workpiece": True,
