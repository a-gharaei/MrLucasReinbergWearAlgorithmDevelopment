...
def save_all_labels_in_TIFF_format(df_original, dir, time_step_size):
    ...
    df = df_original.rename(
        {
            "tool force X": "Tool force X component - N",
            ...
            "Sum of projected cutting areas - micrometer^2": "[µm$^{2}$]",
        },
        axis="columns",
    )
...
def present(result_df, path, config):
    ...
    wp_last = Workpiece.load_from_disk(os.path.join(path, "last_state_workpiece.pkl"))
    ...
    if fracture_wear:
        print(f"{path}/fracture_wear_results.pkl")
        ...
        # show fracture grains
        for grain_mesh in fractured_grains_list:
            mesh = mesh_conversion.from_mesh_to_trimesh(grain_mesh)
            figure = plt.figure()
            axes = figure.add_subplot(projection="3d")
            axes.add_collection3d(
                mplot3d.art3d.Poly3DCollection(
                    mesh.triangles, alpha=0.7, edgecolors="k"
                )
            )
        ...
        highest_pen_depth = max(fracture_wear_model_results[2])
        ...
        ax[1].text(80, 2, f"Highest penetration depth:{highest_pen_depth}")
    ...
    if config["visualize_process"]:
        ...
        fm_plot_config = FlatManifoldPlotConfig(
            PosePlotConfig.default(),
        ...
        plot_process(
            ...
            ProcessPlotConfig(
                StatePlotConfig(
                    ToolPlotConfig.default(tool),
                    WorkpiecePlotConfig([fm_plot_config] * len(wp.slices)),
                ),
                KinematicsPlotConfig.default(),
            ),
            kinematics_idx=0,
    ...
    if config["visualize_forces"]:
        ...
        if config["visualize_forces_interactive"]:
            ...
            result_df[
                ["tool force X", "tool force Y", "tool force Z"]
            ].plot_bokeh.line()
    ...
    if config["visualize_workpiece"]:
        ...
        plot_workpiece(
            wp,
            WorkpiecePlotConfig.default(wp),
