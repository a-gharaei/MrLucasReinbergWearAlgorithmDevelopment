...
def create_wheel(inputs):
    ...
    grinding_wheel = Tool.from_radius_and_height(basic_grain=ibrus_grain,
                                                 inner_radius=inputs['wheel_inner_radius[mm]'],
                                                 outer_radius=inputs['wheel_outer_radius[mm]'],
                                                 height=(inputs['height_from[mm]'], inputs['height_to[mm]']),
...
if __name__ == '__main__':
    ...
    input_df = pd.read_csv(working_dir + '/input_parameters_wheel_with_height.csv')

    if os.path.exists(working_dir + '/CreatedWheelsWithHeight'):
        shutil.rmtree(working_dir + '/CreatedWheelsWithHeight')
    os.makedirs(working_dir + '/CreatedWheelsWithHeight')

    for exp in range(input_df.shape[0]):

        output_path = working_dir + '/CreatedWheelsWithHeight/wheel#' + str(int(input_df['template_wheel_number'][exp]))
        ...
        tool, tool_properties = create_wheel({'grain_stl_path': r'./RailGrinding/Resources/Grains/Cuboctahedron.stl',
                                              ...
                                              'height_from[mm]': input_df['height_from[mm]'][exp].item(),
                                              'height_to[mm]': input_df['height_to[mm]'][exp].item(),
