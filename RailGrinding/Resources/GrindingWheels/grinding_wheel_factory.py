...
def get_wheel_info(grain, tool, inputs, saving_path):
    ...
    if inputs["save_wheel"]:
        ...
        plt.xlabel("volume")
        plt.savefig(
            os.path.join(saving_path, "histogram of grains volume.jpg"),
            dpi=300,
            bbox_inches="tight",
        )
    ...
    average_radius = (
        (inputs["wheel_inner_radius[mm]"] + inputs["wheel_outer_radius[mm]"])
        * 0.5
        * 0.001
    )  # [m]

    info_dict = {
        "average_radius[m]": average_radius,
...
def create_wheel(inputs):
    if inputs["save_wheel"]:
        saving_path = r"./RailGrinding/Resources/GrindingWheels/wheel#" + str(
            int(inputs["template_wheel_number"])
        )
    ...
    ibrus_grain = Grain.from_stl(
        inputs["grain_stl_path"], Pose.identity(), Pose.identity()
    )

    # Generate tool that is using this grain shape
    wheel_creation_time0 = time.time()
    grinding_wheel = Tool.from_radius(
        basic_grain=ibrus_grain,
    ...
    calculated_info.update(
        get_wheel_info(ibrus_grain, grinding_wheel, inputs, saving_path=saving_path)
    )

    wheel_properties = WheelProperties(
        wheel_inner_radius=inputs["wheel_inner_radius[mm]"],
        wheel_outer_radius=inputs["wheel_outer_radius[mm]"],
        number_of_grains=inputs["number_of_grains[-]"],
        random_seed_number=inputs["random_seed_number[-]"],
        average_grains_central_penetration=calculated_info[
            "average_grains_volume[mm3]"
        ],
    ...
    if inputs["save_wheel"]:
        ...
        historical_data.save_to_disk_as_pkl(
            {file_name: grinding_wheel}, path=os.path.join(saving_path)
        )
        ...
        raise Exception(
            "Wrong save_wheel input! Correct inputs are 'False' or a two-digit integer number"
        )
...
if __name__ == "__main__":
    working_dir = "Resources/GrindingWheels"
    ...
    for exp in range(input_df.shape[0]):
        ...
        output_path = (
            working_dir
            + "/CreatedWheels/wheel#"
            + str(int(input_df["template_wheel_number"][exp]))
        )
        ...
        tool, tool_properties = create_wheel(
            {
                ...
                "number_of_grains[-]": int(input_df["number_of_grains[-]"][exp].item()),
                "random_seed_number[-]": int(
                    input_df["random_seed_number[-]"][exp].item()
                ),
                "save_wheel": bool(input_df["save_as_template"][exp]),
                "template_wheel_number": input_df["template_wheel_number"][exp].item(),
            }
        ...
        historical_data.save_to_disk_as_pkl(
            {str("wheel#" + str(int(input_df["template_wheel_number"][exp]))): tool},
            path=output_path,
        )
