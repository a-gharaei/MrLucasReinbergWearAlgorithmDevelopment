...
from AnalyzeSimulationResults import historical_data
from PhysicalModels.controller import *
from PhysicalModels.force_controlled_kinematics import (
    ProportionalController,
    ProportionalControllerResultList,
)
from PhysicalModels.kienzle_force_model import *
from PhysicalModels.takazawa_temperature_model import *
from PhysicalModels.Usui_wear_model import *
from Resources.GrindingWheels.grinding_wheel_factory import create_wheel
from wear_models import apply_fracture_wear_model
...
def process_simulation(input_df: DataFrame, output_path: str):
    ...
    for exp in range(input_df.shape[0]):
        ...
        if bool(input_df["load_tool_from_disk"][exp]):
            ...
            # exp])+'.pkl')
            loading_directory = r"Resources/GrindingWheels/wheel#" + str(
                int(input_df["wheel_number_to_load"][exp])
            )
            tool = Tool.load_from_disk(
                loading_directory
                + "/wheel#"
                + str(int(input_df["wheel_number_to_load"][exp]))
                + ".pkl"
            )
            tool_properties_df = pd.read_csv(
                loading_directory + "/wheel properties.csv"
            )
            ...
            tool, tool_properties = create_wheel(
                {
                    "grain_stl_path": r"Resources/Grains/Cuboctahedron.stl",
            ...
            )  # a none-zero integer number)
            tool_properties_df = tool_properties.get_as_dataframe()
            sim_params = {"tool_df": tool_properties_df}

        historical_data.save_to_disk_as_pkl(
            {"first_state_tool": tool},
            path=output_path + "/Exp" + str(exp) + "/PKLFiles",
        )
        tool_properties_df.to_csv(
            output_path + "/Exp" + str(exp) + "/PKLFiles" + "/wheel_properties.csv"
        )
        ...
        if bool(input_df["load_workpiece_from_disk"][exp]):
            workpiece_load_directory = (
                r"./RailGrinding/Resources/Workpieces/workpiece#"
            ...
            historical_data.save_to_disk_as_pkl(
                {"first_state_workpiece": wp},
                path=output_path + "/Exp" + str(exp) + "/PKLFiles",
            )
        else:
            box = Box.from_sizes(
                size_x=input_df["wp_x[mm]"][exp],
                size_y=input_df["wp_y[mm]"][exp],
                size_z=input_df["wp_z[mm]"][exp],
                pose=Pose.from_translation(
                    Vector(0, -input_df["wp_y[mm]"][exp] / 2, 0)
                ),
            )
            wp = Workpiece.from_box(
                box, spatial_resolution=input_df["wp_spatial_resolution[mm]"][exp]
            )
            historical_data.save_to_disk_as_pkl(
                {"first_state_workpiece": wp},
                path=output_path + "/Exp" + str(exp) + "/PKLFiles",
        ...
        workpiece_material_propertise = WorkpieceMaterialProperties(
            ...
            thermal_conductivity=float(input_df["thermal_conductivity[W/mK]"][exp]),
        ...
        plot_workpiece(
            wp, WorkpiecePlotConfig.default(wp), length_unit="mm", dpi="5000"
        )
        ...
        print(input_df["wheel_center_z_position[mm]"][exp])
        start_point = Vector(
            input_df["wheel_center_x_position[mm]"][exp],
        ...
        rotation_axis = (
            Vector.e_z()
            .transform(
                Transform.from_axis_angle(
                    Vector.e_y(), (np.pi / 180) * input_df["approach_angle[deg]"][exp]
                )
            )
            .transform(
                Transform.from_axis_angle(
                    Vector.e_x(), (np.pi / 180) * input_df["tilt_angle[deg]"][exp]
                )
            )
        )
        # "rad/sec"
        ...
        time_step_size = 1 / (
            input_df["simulation_step_size[steps/rev]"][exp]
            * input_df["rotational_speed[Hz]"][exp]
        )  # 60 [rev/s] --> 1/(360* 60) = 4.63e-5
        ...
        plot_process(
            process,
            ProcessPlotConfig.default(process),
            axis_labels=["X", "Y", "Z"],
            length_unit="[mm]",
            dpi="1000",
        )
        ...
        print(
            "process_creation_time: ", process_creation_time1 - process_creation_time0
        )
        ...
        force_model_config = GrainForceModelConfiguration(
            specific_cutting_force=spec_cutting_force,
        ...
        if bool(input_df["controller_model"][exp]):
            cont = PidController(
                parameter_i=input_df["parameter_i"][exp].item(),
                ...
                controller_timing=int(input_df["controller_timing[s]"][exp].item()),
                change_in_z_position_by_controller=input_df[
                    "change_in_z_position_by_controller[mm]"
                ][exp].item(),
            )  # every x steps

        if bool(input_df["force_controlled"][exp]):
            cont = ProportionalController(
                proportional_gain=float(input_df["proportional_gain[mm]"][exp])
            )  # every x steps
        ...
        print("sim_preparation_time: ", sim_preparation_time1 - sim_preparation_time0)
        ...
        for current_tool_pose in kin.ToolTrajectory.poses:
            ...
            if bool(input_df["temperature_model"][exp]):
                grain_temperature_model_result = grain_temperature_model(
                    mat_remover_result,
            ...
            if bool(input_df["wear_model"][exp]):
                ...
                apply_attritious_wear(
                    ...
                    tool,
                    mat_remover_result,
                    input_df["use_reduced_grain"][exp],
                )
            ...
            # --------------- controller
            if bool(input_df["controller_model"][exp]):
                controller_result = cont.get_controller_results(
                    rotational_speed,
            ...
            if bool(input_df["force_controlled"][exp]):
                controller_result = cont.get_proportional_controller_results(
                    tool_force_results,
                    kin.ToolTrajectory,
                    sim_i,
                    int(input_df["controller_timing[No.Steps]"][exp].item()),
                    float(input_df["reference_normal_force[N]"][exp].item()),
                    float(input_df["wear_compensation[mm]"][exp].item()),
                )
                if (
                    (sim_i + 1)
                    % int(input_df["controller_timing[No.Steps]"][exp].item())
                    ...
                    print("current_z_position", controller_result.current_z_position)
                    print(
                        "next_wheel_z_position", controller_result.next_wheel_z_position
                    )
        ...
        if bool(input_df["save_force_results"][exp]):
            simulation_results_grain_forces = (
                GrainForceModelResultListNumpyArrays.from_grain_force_model_result_list(
                    grain_force_results
                )
            )
            simulation_results_tool_forces = (
                ToolForceModelResultListFloat.from_tool_force_model_result_list(
                    tool_force_results
                )
            )
            # for release 1.2.0
            ...
            simulation_results_tool_global_forces = (
                simulation_results_tool_forces.get_forces_in_global(kin.ToolTrajectory)
            )
        ...
        historical_data.save_to_disk_as_pkl(
            {
                ...
                "simulation_tool_forces_global_frame": simulation_results_tool_global_forces,
                ...
                "simulation_material_removal_results": simulation_material_removal_results,
            },
            path=output_path + "/Exp" + str(exp) + "/PKLFiles",
        )
