...
def macro_fracture(grain, F_C, F_N, P_DEPTH, cutting_dir, tensile_strength):
    if not isinstance(grain, Grain3D):
        ...
        grain3d = grain
        grain_type = "trimesh"
    if not grain3d.mesh.is_watertight:
        print("mesh is not watertight")
    if isinstance(cutting_dir, Vector):
        cutting_direction = geom.Vector(
            cutting_dir.x(), cutting_dir.y(), cutting_dir.z()
        )
    else:
        cutting_direction = cutting_dir
    ...
    if P_DEPTH > (grain3d.maxz - grain3d.minz) / 3:
        ...
        return 0, rankine
    initial_volume = rotated_mesh.volume
    ...
    # Convert to iBrus mesh type
    if grain_type == "iBrus":
        new_mesh = mesh_conversion.from_trimesh_to_mesh(
            fractured_mesh, original_mesh_pose
        )
        # Update mesh in the simulation
        grain = grain.update_mesh(new_mesh)
    else:
        grain3d.mesh = fractured_mesh
